...
- Only development/self-hosting composition activates stock `agents_md_plugin`; Chat activates no source and remains AGENTS-unaware. Each snapshot rereads root `AGENTS.md` through the Session-authorized `AuthorizedEnvironmentFileReadFacet`, not a Project or host filesystem fallback. `not_found` and blank files are successful empty results; other read/service/authority errors fail the required source. Exact nonblank text and its Environment revision form one material, separate from stable plugin-owned explicit-user-precedence semantics. Generic instruction-only composition and independent Skills/role/map ownership are unchanged.
