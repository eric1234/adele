...
Accepted; B1 Project opening, normal Task/primary Environment creation, in-memory Session-bound execution, headless stock Chat, Environment-authorized tools, and instruction-context capture implemented, broader lifecycle deferred
...
- `app/lib/core/adele_runtime.dart` owns the shared capability/extension registries, in-memory product store, generated lifecycle coordinator, inference context composer, retained Chat plugin, and five static in-process activations: Chat, Filesystem, Search, Command, and Local Directory Project Selector. All use the same extension registry and retire in reverse order; reduced composition omits only Command Tools. `AdeleRuntime()` remains synchronous and provider-free and owns pure-Dart `ApplicationPluginBootstrap` on those same registries, without starting it from the constructor. AGENTS.md is now an AOT backend rather than a direct app import/dependency or static activation.
- Normal `AdeleApplication` explicitly starts generic application-lifetime bootstrap with a prepared installation root, shared host/runtime paths, and optional generic argv. It attempts all valid backend/frontend components from one catalog without stock activation callbacks or required-Git/optional-OpenAI tiers. Local backend failure rolls back both capability and extension registrations for that attempt; shared-host failure affects all backends. Missing live Environment support leaves Task creation unavailable, not Project opening. Application close blocks actions/notifications, drains Task establishment and only currently advancing Run start/resume even on failure, and abandons quiescent waiting Runs without executing pending invocations. This adds no cancellation or rollback. Backend registrations retire before connections, then the host, then in-process activations; every cleanup action is attempted.
- Repository tooling owns stock preparation: six installations, three backend snapshots plus the host and four EVCs, using the same four generic deployment defines and no AGENTS.md configuration. Backend entrypoints own ready capability/extension exposures; generic activation uses existing registries, with no app exposure helpers or implementation imports. Self-hosting retains its independent backend/product/Run topology, adding explicit `agentsMdArtifact` on the same host through the same source adapter without a normal installation root. Normal startup creates no product values, tool catalog, or Run. Profiles remain unimplemented; current ownership is maintained in `docs/architecture/overview.md`.
...
- Task presentation calls `runtime.lifecycle.createTask(projectId: ..., title: ...)` without a provider ID. Capability defaults remain descending rank then ascending provider identity; providers own source validation, including non-Git rejection. Selection is window-local, updated only on lifecycle success and guarded against late completion after disposal/exit. Presentation neither parses opaque `providerState` nor restores/migrates a generation to report readiness.
- Normal presentation supports Task/primary Environment creation and one canonical stock Chat Session with fresh approval-gated Runs per prompt. Session validity is independent of model availability; provider binding, Session-authorized tools, and policy are selected per Run. Window-local cards resolve exact retained Run interruptions, not canonical Chat entries; approval preserves revision checks and generation-bound authority and is not sandboxing. Kernel/public APIs, Chat sequencing, and Environment contracts are unchanged. Configurable permissions/profiles, general provider/model configuration, Task Browser, and rich workbench UI remain deferred.
...
- The application has an in-memory Task establishment coordinator that publishes a Task and finalized primary Environment together and records the exact establishment-time materialization only after provider success. It intentionally retains successful provider state even if the generation retires immediately afterward; stale live readiness does not undo successful publication. Task presentation changes neither lifecycle settlement nor generation semantics; disk persistence remains deferred.
...
- Normal prepared startup and explicit self-hosting activate `agents_md_backend` through generic remote extension adapters, reusing pure-Dart `agents_md_plugin` semantics; Chat activates no source and remains AGENTS-unaware. Activation alone does not read a file. Each snapshot rereads root `AGENTS.md` through generated authorized reads backed by the captured canonical Session's `AuthorizedEnvironmentFileReadFacet`, not authority reconstructed from transported IDs or a host filesystem fallback. Opaque per-operation host contexts are allowlisted and revoked at settlement/retirement/termination. `not_found` and blank files succeed empty; other read/service/authority errors fail the required source. Exact text/revision and separate plugin-owned explicit-user-precedence semantics are unchanged, as are generic composition and independent Skills/role/map ownership. This is not general symmetric RPC or a sandbox.
- Nested/scoped AGENTS.md, aliases/overrides, global/home files, imports, AGENTS.md caching, other context sources, broader Reference/Observation material, provider-aware projection/cache planning, token budgets, compaction, rich Chat UI/persistence, child Session lifecycle, strategy defaults, profiles, Task Browser, and disk persistence remain deferred. Complete Session lifecycle and restoration are not implemented.
