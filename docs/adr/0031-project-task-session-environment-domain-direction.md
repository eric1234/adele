...
- Normal presentation supports Task/primary Environment creation and one canonical stock Chat Session with fresh read-only Runs per prompt. Session validity is independent of model availability; provider binding, Session-authorized tools, and policy are selected per Run. General provider/model configuration, Task Browser, and rich workbench UI remain deferred.
