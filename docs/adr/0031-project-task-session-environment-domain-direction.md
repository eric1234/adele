...
- Normal `AdeleApplication` explicitly starts generic application-lifetime bootstrap with prepared artifacts and composition-supplied callbacks for one shared backend host. Missing configuration or startup failure leaves Task support unavailable, not Project opening; failed startup cleans up before reporting the original error. Application close drains pending Task establishment before runtime cleanup even on failure, without cancellation or rollback. Backend registrations retire before connections, then the host, then in-process activations; every cleanup action is attempted.
- Stock selection stays at the application composition edge. `app/lib/plugins/stock_git_environment.dart` shares Git identities, exposure, and configuration-context registration between normal and self-hosting paths through public Environment contracts and host APIs, not backend implementation imports. Self-hosting retains its independent backend and product/Run topology. Normal startup activates no model provider and creates no product values, tool catalog, or Run.
...
- Beyond Project opening, normal presentation supports only Task/primary Environment creation. General provider/model configuration, Task Browser, Session/Chat UI, and the Run product flow remain deferred.
