...
- `app/lib/core/adele_runtime.dart` owns the shared capability/extension registries, in-memory product store, generated lifecycle coordinator, inference context composer, and retained Chat plugin. `AdeleRuntime()` is synchronous and provider-free. Normal application bootstrap registers backend capabilities and extensions in those same registries; AGENTS.md is an installed AOT backend, not a static runtime activation. Startup creates no product values, tool catalog, or Run. Current composition and deployment ownership are maintained in [the architecture overview](../architecture/overview.md).
- Missing live Environment support leaves Task creation unavailable, not Project opening. Local backend activation failure rolls back that attempt's capability and extension registrations; shared-host failure affects all backends. Application close blocks actions/notifications, drains Task establishment and currently advancing Run start/resume even on failure, and abandons quiescent waiting Runs without executing pending invocations. This adds no cancellation or rollback.
...
- AGENTS.md is a required remote inference-context source in normal prepared startup and explicit self-hosting; Chat activates no source and remains AGENTS-unaware. Activation alone does not read a file. Each snapshot rereads root `AGENTS.md` through the captured canonical Session's `AuthorizedEnvironmentFileReadFacet`. Session Environment authority remains host-owned: transported IDs cannot select it, and there is no Project or host filesystem fallback. `not_found` and blank files succeed empty; other read/service/authority errors fail the source. Exact text/revision and separate plugin-owned explicit-user-precedence semantics preserve generic composition and independent Skills/role/map ownership. [ADR 0032](0032-remote-backend-extensions-use-operation-scoped-host-services.md) records the remote host-service decision.
