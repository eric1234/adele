...
- Normal `AdeleApplication` explicitly starts generic application-lifetime bootstrap with prepared artifacts and composition-supplied callbacks for one shared backend host. Missing required host/Git configuration or failed required startup leaves Task support unavailable, not Project opening; failed required startup cleans up before reporting the original error. Optional OpenAI failure affects model availability only. Application close synchronously blocks actions and notifications, drains pending Task establishment and only the currently advancing Run start/resume before runtime cleanup even on failure, and abandons quiescent waiting Runs without resolving or executing their pending invocations or waiting indefinitely. This adds no cancellation or rollback. Backend registrations retire before connections, then the host, then in-process activations; every cleanup action is attempted.
- Stock selection stays at the application composition edge. Stock Git and OpenAI helpers share identities, exposure, and configuration-context registration between normal and self-hosting paths through public contracts and host APIs, not backend implementation imports. Self-hosting retains its independent backend and product/Run topology. Normal startup activates Git and independently activates experimental ChatGPT when configured; it creates no product values, tool catalog, or Run. OpenAI failure does not retire Git, while shared-host failure remains global.
