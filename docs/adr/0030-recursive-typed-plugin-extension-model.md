...
- remote model-tool contributions use generated public `adele_model_tool` transport and an app host adapter over the same registry; Search and Filesystem Tools run as installed AOT backends reusing their pure-Dart semantics, while Chat, Command Tools, and Local Directory Project Selector remain in process;
