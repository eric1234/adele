...
- stock Local Directory Project Selector uses the existing in-process activation convention on the same `AdeleRuntime` registry, not a capability/default-provider route; native implementation details are recorded in `docs/direction/stock-plugin-topology.md`;
...
- remote model-tool contributions use generated public `adele_model_tool` transport and an app host adapter over the same registry; Search, Filesystem Tools, and Command Tools run as installed AOT backends reusing their pure-Dart semantics, while Chat and Local Directory Project Selector remain in process;
...
- source freshness is source-owned without a generic refresh API; normal prepared startup and explicit self-hosting load `agents_md_backend` through remote extension adapters on the existing registry, without a static app activation. Each snapshot rereads root `AGENTS.md` through the captured Session's authorized Environment read service; Chat activates no source and remains AGENTS-unaware;
- ready backend extension advertisements become exact-generation host proxies in the existing registry, with coherent capability/extension rollback and retirement. Model-tool `hostServices` metadata requests supported dependencies, not grants or Profiles; host-captured Session authority is supplied only for an operation. [ADR 0032](0032-remote-backend-extensions-use-operation-scoped-host-services.md) records the operation-scoped host-service decision and its implemented subset;
