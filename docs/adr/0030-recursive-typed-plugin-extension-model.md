...
- source freshness is source-owned without a generic refresh API; normal prepared startup and explicit self-hosting load `agents_md_backend` through generic remote extension adapters on the existing registry, reusing pure-Dart `agents_md_plugin` semantics without a static app activation. Each snapshot rereads root `AGENTS.md` through generated authorized reads backed by the captured Session's `AuthorizedEnvironmentFileReadFacet`; Chat activates no source and remains AGENTS-unaware;
- F3a adds ready `AdeleExtensionExposure` metadata without PluginId or priority, internal host adapter selection, coherent capability/extension rollback and retirement, and operation-scoped unary host reads with exact host-stamped generations and revoked opaque contexts. This is not another contribution registry, general symmetric RPC, reverse streaming, profiles, or a sandbox; current details are maintained in `docs/architecture/contracts-and-capabilities.md`;
