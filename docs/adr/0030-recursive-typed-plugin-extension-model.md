...
- remote model-tool contributions use generated public `adele_model_tool` transport and an app host adapter over the same registry; Search runs as an installed AOT backend reusing its pure-Dart semantics, while Chat, Filesystem Tools, Command Tools, and Local Directory Project Selector remain in process;
