...
final class DartContractEmitter {
  ...
  void _client(
    ...
    for (final MethodModel method in service.methods) {
      ...
      if (method.kind == MethodKind.unary) {
        ...
        out.writeln(
          '@override Stream<${method.returnType.dart}> ${method.name}(${method.parameters.map((FieldModel p) => '${p.type.dart} ${p.name}').join(',')}) => AdeleLazyStream<${method.returnType.dart}>((_adeleOnData0, _adeleOnError1, _adeleOnDone2, _adeleCancelOnError3) { final _adeleStreamChannel4 = this._adeleChannel; if (_adeleStreamChannel4 is! AdeleStreamChannel) throw StateError(${_literal('This generated method requires an AdeleStreamChannel.')}); final $raw = _adeleStreamChannel4.stream(${_lower(service.name)}${_cap(method.name)}Id, <String, Object?>{${method.parameters.map((FieldModel p) => '${_literal(p.id)}: ${_encode(p.type, p.name)}').join(',')}}); return $raw.map<${method.returnType.dart}>((Object? _adeleItem5) => ${_decode(method.returnType, '_adeleItem5', method.name)}).handleError((Object $error) { if ($error is AdeleRemoteFailure) { ${_streamFailureSwitch(failures, error)} } throw $error; }).listen(_adeleOnData0, onError: _adeleOnError1, onDone: _adeleOnDone2, cancelOnError: _adeleCancelOnError3); });',
        );
      }
    }
    out.writeln('}');
  ...
  void _dispatcher(
    ...
    if (streams.isEmpty) {
      out.writeln(
        '@override Future<void> handle(Map<Object?, Object?> _adeleCommand0, void Function(Map<String, Object?>) _adeleSend1) => _adeleExecutor.run<void>(() async => _adeleSend1(await dispatch(_adeleCommand0))); @override Future<void> close() async {} }',
      );
    } else {
      out.writeln(
        '@override Future<void> handle(Map<Object?, Object?> _adeleCommand0, void Function(Map<String, Object?>) _adeleSend1) async { final _adeleKind2 = _adeleCommand0[\'kind\']; if (_adeleKind2 == \'request\') { await _adeleExecutor.run<void>(() async => _adeleSend1(await dispatch(_adeleCommand0))); return; } if (_adeleKind2 == \'streamOpen\') { await _adeleExecutor.run<void>(() => _adeleOpenStream(_adeleCommand0, _adeleSend1)); return; } final _adeleRequestId3 = _adeleCommand0[\'requestId\']; if (_adeleRequestId3 is! int) return; final _adeleState4 = _adeleStreams[_adeleRequestId3]; if (_adeleKind2 == \'streamCredit\') { final _adeleCredit5 = _adeleCommand0[\'credit\']; if (_adeleState4 != null && _adeleCredit5 is int && _adeleCredit5 > 0) { _adeleState4.credit += _adeleCredit5; _adelePump(_adeleState4, _adeleSend1); } } else if (_adeleKind2 == \'streamCancel\' && await _adeleCancel(_adeleRequestId3)) { _adeleSend1({\'kind\': \'streamCancelled\', \'requestId\': _adeleRequestId3}); } }',
      );
      out.writeln(
        'Future<void> _adeleOpenStream(Map<Object?, Object?> _adeleRequest0, void Function(Map<String, Object?>) _adeleSend1) async { final _adeleRequestId2 = _adeleRequest0[\'requestId\']; if (_adeleClosed || _adeleRequestId2 is! int || _adeleStreams.containsKey(_adeleRequestId2)) { _adeleSend1(_contractStreamFailure(_adeleRequestId2, null, \'invalid_request\', \'Malformed stream-open request.\', const {})); return; } late final String _adeleMethod3; late final Map<Object?, Object?> _adelePayload4; late final List<Object?> _adeleArguments5; try { _adeleMethod3 = _decodeContractEnvelope(_adeleRequest0, \'streamOpen\'); if (!const $streamIds.contains(_adeleMethod3)) { final _adeleWrongKind6 = const $unaryIds.contains(_adeleMethod3); _adeleSend1(_contractStreamFailure(_adeleRequestId2, null, _adeleWrongKind6 ? \'wrong_method_kind\' : \'unknown_method\', _adeleWrongKind6 ? \'Unary method requires request.\' : \'Unknown method.\', const {})); return; } _adelePayload4 = _contractMap(_adeleRequest0[\'payload\'], \'request payload\'); _adeleArguments5 = switch (_adeleMethod3) {',
      );
      ...
      out.writeln(
        "_ => throw const _ContractUnknownMethod(), }; } on AdeleProtocolException catch(_adeleError7) { _adeleSend1(_contractStreamFailure(_adeleRequestId2, null, 'invalid_request', _adeleError7.message, const {})); return; } try { final Stream<Object?> _adeleSource8 = switch (_adeleMethod3) {",
      );
      for (final MethodModel method in streams) {
        out.writeln(
      ...
      out.writeln(
        "_ => throw const _ContractUnknownMethod(), }; final _adeleState9 = _ContractStreamState(_adeleRequestId2, _adeleMethod3, AdeleStreamIterator<Object?>(_adeleSource8)); _adeleStreams[_adeleRequestId2] = _adeleState9; } on Object { _adeleSend1(_contractStreamFailure(_adeleRequestId2, null, 'internal_error', 'The backend stream failed unexpectedly.', const {})); } }",
      );
      out.writeln(
        'Future<void> _adelePump(_ContractStreamState _adeleState0, void Function(Map<String, Object?>) _adeleSend1) async { if (_adeleState0.pumping || _adeleState0.done) return; _adeleState0.pumping = true; try { while (!_adeleState0.done && _adeleState0.credit > 0) { _adeleState0.credit--; late final bool _adeleHasItem2; try { _adeleHasItem2 = await _adeleState0.iterator.moveNext(); }',
      );
      for (final FailureModel failure in failures) {
        out.writeln(
      ...
      for (final MethodModel method in streams) {
        out.writeln(
          '${_lower(service.name)}${_cap(method.name)}Id => ${_encode(method.returnType, '(_adeleState0.iterator.current as ${method.returnType.dart})')},',
        );
      }
      out.writeln(
        "_ => throw const _ContractUnknownMethod(), }; _adeleSend1({'kind': 'streamItem', 'requestId': _adeleState0.requestId, 'payload': _adeleEncoded4}); } on Object { _adeleFinish(_adeleState0, _adeleSend1, _contractStreamFailure(_adeleState0.requestId, null, 'backend_contract_violation', 'The backend violated its generated contract.', const {})); return; } } } finally { _adeleState0.pumping = false; if (!_adeleState0.done && _adeleState0.credit > 0) _adelePump(_adeleState0, _adeleSend1); } } void _adeleFinish(_ContractStreamState _adeleState0, void Function(Map<String, Object?>) _adeleSend1, Map<String, Object?> _adeleTerminal2) { if (_adeleState0.done || _adeleStreams.remove(_adeleState0.requestId) != _adeleState0) return; _adeleState0.done = true; _adeleSend1(_adeleTerminal2); } Future<bool> _adeleCancel(int _adeleRequestId0) async { final _adeleState1 = _adeleStreams.remove(_adeleRequestId0); if (_adeleState1 == null || _adeleState1.done) return false; _adeleState1.done = true; await _adeleState1.iterator.cancel(); return true; } @override Future<void> close() async { if (_adeleClosed) return; _adeleClosed = true; final _adeleIds0 = _adeleStreams.keys.toList(growable: false); for (final _adeleId1 in _adeleIds0) { await _adeleCancel(_adeleId1); } } } String _decodeContractEnvelope(Map<Object?, Object?> _adeleRequest0, String _adeleKind1) { _contractFields(_adeleRequest0, const {'kind', 'requestId', 'method', 'payload'}, 'request envelope'); if (_adeleRequest0['requestId'] is! int || _adeleRequest0['kind'] != _adeleKind1 || _adeleRequest0['method'] is! String) throw const AdeleProtocolException('Malformed request envelope.'); return _adeleRequest0['method'] as String; } final class _ContractUnknownMethod implements Exception { const _ContractUnknownMethod(); } final class _ContractStreamState { _ContractStreamState(this.requestId, this.method, this.iterator); final int requestId; final String method; final AdeleStreamIterator<Object?> iterator; int credit = 0; bool pumping = false; bool done = false; } ",
      );
    }
    if (streams.isEmpty) {
      out.writeln(
