...
part of 'remote_inference_context.dart';

const String remoteInferenceContextSourceServiceId = 'inferenceContextSource';
const String remoteInferenceContextSourceServiceSnapshotId =
    'inferenceContextSource.snapshot';

final class RemoteInferenceContextSourceServiceClient
    implements RemoteInferenceContextSourceService {
  const RemoteInferenceContextSourceServiceClient(
  ...
  Future<List<RemoteInferenceInstruction>> snapshot(
    ...
    try {
      return List<RemoteInferenceInstruction>.unmodifiable(
        _contractList(
          await this._adeleChannel.request(
            remoteInferenceContextSourceServiceSnapshotId,
            <String, Object?>{
              'sessionId': sessionId,
              'runId': runId,
          ...
          (_adeleElement7) => _decodeRemoteInferenceInstruction(_adeleElement7),
        ),
      );
...
abstract interface class RemoteInferenceContextSourceServiceRequestDispatcher
    implements AdeleBackendDispatcher {}

final class RemoteInferenceContextSourceServiceDispatcher
    implements RemoteInferenceContextSourceServiceRequestDispatcher {
  RemoteInferenceContextSourceServiceDispatcher(this._adeleService);
  final RemoteInferenceContextSourceService _adeleService;
  ...
  Future<Map<String, Object?>> _adeleDispatchCore(
    ...
    try {
      _adeleResult8 = await switch (_adeleMethod2) {
        remoteInferenceContextSourceServiceSnapshotId => (() async {
          final _adeleValues0 = _adeleArguments6 as List<Object?>;
          return await this._adeleService.snapshot(
    ...
    try {
      final _adeleEncoded11 = switch (_adeleMethod2) {
        remoteInferenceContextSourceServiceSnapshotId =>
          (_adeleResult8 as List<RemoteInferenceInstruction>)
              .map(
                (_adeleElement17) =>
                    _encodeRemoteInferenceInstruction(_adeleElement17),
              )
...
const String remoteInferenceInstructionTypeId =
    'inferenceContextSource.instruction';
Map<String, Object?> _encodeRemoteInferenceInstruction(
  RemoteInferenceInstruction _adeleValue21,
) => <String, Object?>{
  'key': _adeleValue21.key,
  'revision': switch (_adeleValue21.revision) {
    final _adeleNonNullValue25? => _adeleNonNullValue25,
    null => null,
  },
  'text': _adeleValue21.text,
};
RemoteInferenceInstruction _decodeRemoteInferenceInstruction(
  Object? _adeleValue30,
) {
  final _adeleMap31 = _contractMap(_adeleValue30, 'RemoteInferenceInstruction');
  _contractFields(_adeleMap31, const {
    'key',
    'revision',
    'text',
  }, 'RemoteInferenceInstruction');
  final _adeleField32 = _contractString(_adeleMap31['key'], 'key');
  final _adeleField33 = switch (_adeleMap31['revision']) {
  ...
  final _adeleField34 = _contractString(_adeleMap31['text'], 'text');
  return _contractConstruct(
    'RemoteInferenceInstruction',
    () => RemoteInferenceInstruction(
      key: _adeleField32,
      revision: _adeleField33,
      text: _adeleField34,
    ),
  );
}
