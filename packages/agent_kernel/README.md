...
context for plugin-contributed `read_file`, `apply_patch`, `search`, and
`run_command`. Tool policy remains kernel-owned; Filesystem Tools owns patch
interpretation, Command Tools owns command projection and terminal retention,
and the host supplies only the authorized Environment facets.
